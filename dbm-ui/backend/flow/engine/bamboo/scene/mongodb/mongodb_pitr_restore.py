...
class MongoPitrRestoreFlow(MongoBaseFlow):
    ...
    def start(self):
        ...
        for row in self.payload["infos"]:
            ...
            # src_cluster_id 如果 = 0 读取 row["src_cluster"] 这个一般用于测试
    ...
    def process_cluster(self, row: Dict, src_cluster: MongoDBCluster,
                        cluster: MongoDBCluster, actuator_workdir: str, dest_dir: str) -> SubBuilder:
        """
        ...
        self.stop_dbmon(row=row, cluster=cluster, actuator_workdir=actuator_workdir,
                        dest_dir=dest_dir, cluster_sb=cluster_sb)
        ...
        for idx in range(len(src_shards)):
            ...
            shard_pipes.append(
                shard_sb.build_sub_process(_("{} restore {} to {}").format(
                    src_shard.set_type, src_shard.set_name, dst_shard.set_name)))
        ...
        if cluster.is_sharded_cluster():
            ...
            self.rebuild_cluster(
                row=row, actuator_workdir=actuator_workdir, dest_dir=dest_dir,
                src_cluster=src_cluster, dst_cluster=cluster,
    ...
    def process_shard(self, row: Dict, src_cluster, src_shard,
                      cluster, shard, actuator_workdir: str, dest_dir: str, shard_sub: SubBuilder):
        """
    ...
    def rebuild_cluster(
            self, row: Dict, actuator_workdir: str, dest_dir: str,
            src_cluster: MongoDBCluster, dst_cluster: MongoDBCluster,
        ...
        acts_list.append(
            {
                ...
                "act_component_code": ExecJobComponent2.code,
                "kwargs": PitrRebuildSubTask.make_kwargs(exec_node=exec_node, file_path=actuator_workdir,
                                                         src_shard=src_configsvr, dst_shard=dst_configsvr,
