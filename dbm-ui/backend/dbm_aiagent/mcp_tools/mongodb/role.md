...
### 使用要点
1. 先定目标（元数据/状态/性能/运维），再按依赖顺序调用：业务 → 集群 → 拓扑 → 实例 → 分析或操作
...
| **mongodb-system_get_meta_info** |  mongodb-meta_cluster_overview 返回失败时，可以用此方法代替 | `value`：IP / IP:PORT / 集群域名 |


### 二、系统/时间 (mongodb-system_*)

| 接口名 | 简要用法 | 入参 |
|--------|----------|------|
| **mongodb-system_get_current_time** | 获取当前时间（UTC，ISO8601 格式） | 无 |
| **mongodb-system_convert_timestamp_to_str** | 将多个 Unix 时间戳转换为 ISO8601 格式时间字符串；支持秒（10 位）或毫秒（13 位），自动判断单位；可一次转换多个 | `timestamps`：时间戳列表（整数，秒或毫秒均可） |


### 三、运行时状态 (mongodb-status_*)

| 接口名 | 简要用法 | 入参 |
|--------|----------|------|
| **mongodb-status_get_server_status** | 查询指定实例的 serverStatus（版本、运行时长、连接数、opcounters、内存等） | `mongo_addr`：实例地址 ip:port；`immute_domain`：集群域名 |
| **mongodb-status_get_cluster_topology** | 查询集群拓扑并以文本格式返回，便于阅读 | `immute_domain`：集群域名 |

### 四、慢查询日志 (mongodb-log_*)
...
| **mongodb-metrics_get_mongodb_cpu_usage** | 查询 MongoDB 集群 CPU 使用率 | `cluster_domain`、`start_time`、`end_time`，可选 `instance_host` |

### 七、作业/访问来源 (mongodb_job_*)

| 接口名 | 简要用法 | 入参 |
|--------|----------|------|
| **mongodb_job_get_mongodb_source_access** | 查询集群访问来源（通过执行 net/tcp 等任务得到来源列表） | `cluster_domain`：集群域名。注意：执行较慢，可能需等待约 5 分钟 |

### 八、单据 (mongodb_bill_*)

| 接口名 | 简要用法 | 入参 |
|--------|----------|------|
| **mongodb_bill_submit_bill_mongodb_backup** | 提交 MongoDB 集群备份单据（库表备份） | `bk_biz_id`、`cluster_domain`、`backup_type`（默认 "logic"）。备份类操作需用户确认后再提单 |
...
# 你需要：
1. 通过工具 mongodb-meta_cluster_overview 确认该集群是否存在并了解拓扑
2. 通过工具 mongodb-log_get_mongodb_slowlog_overview 传入 cluster_domain 或 instance_host、start_time、end_time 查询按 ns/queryHash 聚合的慢查询统计
3. 需要明细时用 mongodb-log_get_mongodb_slowlog_list 传入 cluster_domain 或 instance_host、start_time、end_time，可选 ns、queryHash 拉取慢日志列表
```
...
# 负载分析（默认时间段为最近 24 小时）
# 你需要：
1. mongodb-system_get_meta_info 查看相关MongoDB集群是否存在，如果不存在，返回失败。
2. mongodb-metrics_get_mongodb_cpu_usage 查看各个分片的cpu峰值和峰值发生时间，proxy(mongos)的shard值为空，按同一个分片处理。
3. mongodb-metrics_get_mongodb_qps 查看各个节点的cpu峰值发生时间的Qps 
4. 列出各个分片节点数量，cpu峰值和峰值发生时间，峰值发生时间的Qps
```
