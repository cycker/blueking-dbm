...
from backend.dbm_aiagent.mcp_tools.mongodb.serializers.query_time import (
...
class MongoSystemMcpToolsViewSet(McpToolsViewSet):
    ...
    @mcp_tools_api_decorator(
        ...
        mcp=[DBMMcpTools.MONGODB_SYSTEM],
        name_prefix=DBMMcpTools.MONGODB_SYSTEM,
    )
    ...
    @mcp_tools_api_decorator(
        description=str(
            _("将多个 Unix 时间戳转换为 ISO8601 格式时间字符串；支持秒（10 位）或毫秒（13 位），会自动判断单位；支持一次转换多个")
        ),
    ...
    @mcp_tools_api_decorator(
        description=str(
            _(
                "根据 IP、IP:PORT 或集群域名查询 MongoDB 实例元数据（cluster_domain、instance_host 等），"
                "是后续指标/告警查询的第一步"
            )
        ),
