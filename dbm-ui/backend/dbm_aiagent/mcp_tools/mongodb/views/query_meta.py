...
class MongoQueryMetaMcpToolsViewSet(McpToolsViewSet):
    ...
    @mcp_tools_api_decorator(
        ...
        mcp=[DBMMcpTools.MONGODB_QUERY_META],
        name_prefix="mongodb_query_meta",
    )
    ...
    def list_mongodb_clusters(self, request, *args, **kwargs):
        ...
        return Response(mongodb_list_clusters(bk_biz_id=bk_biz_id))

    @mcp_tools_api_decorator(
        description=str(
            _(
                "查询指定MongoDB集群的拓扑部署信息，包括集群基本信息、存储实例统计、Mongos/代理实例统计、机器分布等"
            )
        ),
    ...
    def list_cluster_mongos(self, request, *args, **kwargs):
        immute_domain = self.get_param("immute_domain")
        return Response({"mongos": cluster_mongos(immute_domain=immute_domain)})
    ...
    def list_cluster_shards(self, request, *args, **kwargs):
        immute_domain = self.get_param("immute_domain")
        return Response({"shards": cluster_shards(immute_domain=immute_domain)})
